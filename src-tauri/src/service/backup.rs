...
use crate::crypto::{self, decrypt, decrypt_with_aad, encrypt, encrypt_with_aad, EncryptedData};
use crate::database::{
...
pub fn export_vault(
    ...
    crate::validation::export_password(export_password.as_str())?;
    ...
    crate::validation::backup_payload(&payload)?;

    let mut payload_json =
        serde_json::to_vec(&payload).map_err(|e| VaultError::InternalError(e.to_string()))?;
    if payload_json.len() > crate::validation::MAX_BACKUP_DECODED_BYTES {
...
pub fn import_vault(
    ...
    crate::validation::import_password(import_password.as_str())?;
    let lease = state.lease()?;
    ...
    let max_b64_len = crate::validation::MAX_BACKUP_DECODED_BYTES
        .saturating_mul(4)
        .div_ceil(3)
        .saturating_add(4);
    ...
    if ciphertext.len() < 16 || ciphertext.len() > crate::validation::MAX_BACKUP_DECODED_BYTES {
        return Err(VaultError::InvalidBackup(
...
mod tests {
    ...
    fn invalid_kdf_and_oversized_backup_rejected_before_work() {
        ...
        backup.data = "A".repeat(
            crate::validation::MAX_BACKUP_DECODED_BYTES
                .saturating_mul(4)
                .div_ceil(3)
                .saturating_add(5),
        );
