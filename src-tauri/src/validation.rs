//! Shared input validation policy for every application adapter.

use std::collections::HashSet;

use crate::database::Settings;
use crate::{BackupPayload, CreateEntryRequest, UpdateEntryRequest, VaultError};
...
fn invalid(code: &str, message: &str) -> VaultError {
...
pub fn master_password(password: &str) -> Result<(), VaultError> {
...
pub fn export_password(password: &str) -> Result<(), VaultError> {
    master_password(password)
}

pub fn import_password(password: &str) -> Result<(), VaultError> {
...
fn password_size(password: &str) -> Result<(), VaultError> {
...
pub fn entry(request: &CreateEntryRequest) -> Result<(), VaultError> {
    entry_fields(
        &request.title,
...
pub fn entry_update(request: &UpdateEntryRequest) -> Result<(), VaultError> {
    entry_fields(
        &request.title,
        request.url.as_deref(),
        &request.username,
...
fn entry_fields(
    ...
    if title.trim().is_empty() || title.len() > crate::constants::MAX_FIELD_LENGTH {
        return Err(invalid("INVALID_TITLE", "Title is empty or too long"));
    }
    if username.trim().is_empty() || username.len() > crate::constants::MAX_FIELD_LENGTH {
        return Err(invalid("INVALID_USERNAME", "Username is empty or too long"));
    }
    if password.is_some_and(|value| {
        value.is_empty() || value.len() > crate::constants::MAX_PASSWORD_LENGTH
    }) {
        return Err(invalid(
    ...
    if url.is_some_and(|value| {
        value.trim().is_empty() || value.len() > crate::constants::MAX_FIELD_LENGTH
    }) {
        return Err(invalid("INVALID_URL", "URL is empty or too long"));
    }
    if notes.is_some_and(|value| value.len() > crate::constants::MAX_NOTES_LENGTH) {
...
pub fn group_name(name: &str) -> Result<String, VaultError> {
...
pub fn id(value: &str) -> Result<(), VaultError> {
...
pub fn tags(values: &[String]) -> Result<(), VaultError> {
...
pub fn settings(value: &Settings) -> Result<(), VaultError> {
...
pub fn backup_payload(payload: &BackupPayload) -> Result<(), VaultError> {
...
mod tests {
    ...
    fn validation_errors_expose_stable_codes() {
        ...
        assert!(error.public_message().starts_with("PASSWORD_TOO_SHORT:"));
    }
}
